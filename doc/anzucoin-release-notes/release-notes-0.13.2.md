Anzucoin Core version 0.13.2 is now available from:

  <https://download.anzucoin.org/anzucoin-0.13.2.1/>
...
Please report bugs using the issue tracker at github:
  ...
  <https://github.com/anzucoin-project/anzucoin/issues>
...
updates will be released anymore. Without security updates, using a anzucoin
...
ECDSA signatures inside Anzucoin transactions now use validation using
...
Previous versions of Anzucoin Core had their mempool limited by checking
a transaction's fees against the node's minimum relay fee. There was no
upper bound on the size of the mempool and attackers could send a large
number of transactions paying just slighly more than the default minimum
relay fee to crash nodes with relatively low RAM. A temporary workaround
for previous versions of Anzucoin Core was to raise the default minimum
relay fee.

Anzucoin Core 0.13.2 will have a strict maximum size on the mempool. The
...
Anzucoin Core 0.13.2 also introduces new default policy limits on the length and
...
This allows running anzucoind without having to do any manual configuration.
...
Anzucoin Core has a heuristic 'priority' based on coin value and age. This
calculation is used for relaying of transactions which do not pay the
minimum relay fee, and can be used as an alternative way of sorting
transactions for mined blocks. Anzucoin Core will relay transactions with
...
entirely in Anzucoin Core 0.13, and it is at this time undecided whether the
...
Anzucoin Core has been updated to make use of this.

This means that if Tor is running (and proper authorization is available),
Anzucoin Core automatically creates a hidden service to listen on, without
manual configuration. Anzucoin Core will also use Tor automatically to connect
to other .onion nodes if the control socket can be successfully opened. This
will positively affect the number of available .onion nodes and their usage.

This new feature is enabled by default if Anzucoin Core is listening, and
...
Anzucoind can now (optionally) asynchronously notify clients through a
...
(or `settxfee <n>` rpc during runtime). A value of `n=0` signals Anzucoin
Core to use floating fees. By default, Anzucoin Core will use floating
fees.
...
(default: `0.0002` ANZU/kB).

At all times, Anzucoin Core will cap fees at `-maxtxfee=<x>` (default:
0.10) ANZU.
Furthermore, Anzucoin Core will never create transactions paying less than
...
`anzucoin.conf`, where `N` is the number of MiB to allot for
raw block & undo data.
...
caching. A sample config for apache2 could look like:
    ...
    <Location /anzucoinrpc>
        ...
        # optional bypass anzucoind rpc basic auth
        # RequestHeader set Authorization "Basic <hash>"
        # get the <hash> from the shell with: base64 <<< anzucoinrpc:<password>
    </Location>
...
Restarting anzucoind will no longer clear out the list of banned peers; instead
...
- Adding `dbcache=100` in anzucoin.conf
...
anzucoin-cli: arguments privacy
------------------------------
...
For example:
    ...
    $ src/anzucoin-cli -stdin walletpassphrase
    mysecretcode
    120
...
Various code modernizations have been done. The Anzucoin Core code base has
...
- `anzucoin-${VERSION}-arm-linux-gnueabihf.tar.gz`: Linux binaries for the most
  common 32-bit ARM architecture.
- `anzucoin-${VERSION}-aarch64-linux-gnu.tar.gz`: Linux binaries for the most
...
for a new opcode in the Anzucoin scripting system that in combination with
...
Anzucoin transactions currently may specify a locktime indicating when
...
HD wallets are incompatible with older versions of Anzucoin Core.
...
The two phases are distinct in the Anzucoin-Qt GUI. During the first one,
...
- **Elimination of unwanted transaction malleability:** Segregating the witness
  ...
  malleability, which is a problem that makes programming Anzucoin wallet
  software more difficult and which seriously complicates the design of smart
  contracts for Anzucoin.
...
- **Weighting data based on how it affects node performance:** Some parts of
  each Anzucoin block need to be stored by nodes in order to validate future
...
- **Signature covers value:** A simple improvement in the way signatures are
  ...
  of anzucoins they think they are spending, and by having full nodes refuse to
  accept those signatures unless the amount of anzucoins being spent is exactly
...
- **Increased security for multisig:** Anzucoin addresses (both P2PKH addresses
  ...
  believed to be provided by Anzucoin's choice of parameters for its Elliptic
...
- **More efficient almost-full-node security** Satoshi Nakamoto's original
  ...
  produce an accurate copy of Anzucoin's current ledger (called the UTXO set),
  ...
  note that Anzucoin Core does not provide an option to use this capability as
  of this 0.13.2 release.

- **Script versioning:** Segwit makes it easy for future soft forks to allow
  Anzucoin users to individually opt-in to almost any change in the Anzucoin
  Script language when those users receive new transactions.  Features
  currently being researched by Bitcoin and Anzucoin Core contributors that may
...
segwit's start date of 1 January 2017 miners can update the Anzucoin
client to Anzucoin Core 0.13.2 to signal for segwit support. When a
...
Since Anzucoin Core 0.10.0, nodes have defaulted to only relaying and
...
- `anzucoin-${VERSION}-arm-linux-gnueabihf.tar.gz`: Linux binaries targeting
  the 32-bit ARMv7-A architecture.
- `anzucoin-${VERSION}-aarch64-linux-gnu.tar.gz`: Linux binaries targeting
